...
class ResizerGifBackround
{
    use Logger;

    function fire($job, $data)
    {
        $this->logDebug('ResizerGifBackround@fire', [
            'data' => $data,
        ]);
        ...
        exec('sudo su www-data -s /bin/bash -c "convert ' . $data['file'] . ' -verbose -coalesce -define webp:low-memory=true -define webp:method=3 -define webp:thread-level=0 -resize 1920x250^ -quality 90 ' . $fileName . '"');
        // $size_w = 370;
        // $size_h = 370;
